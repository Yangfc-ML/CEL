...
def load_cars196(args):
    ...
    original_train = dsets.StanfordCars(root='../data', split='train', transform=test_transform, download=False)
    ...
    test_dataset = dsets.StanfordCars(root='../data', split='test', transform=test_transform)
    test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=args.bs, shuffle=False, num_workers=4)

    model = models.wide_resnet50_2()
    model.fc = nn.Linear(model.fc.in_features, max(ori_labels) + 1)
    model = model.cuda()
    model.load_state_dict(
        torch.load(os.path.expanduser('weight/cars196/cars196.pt'))['model_state_dict'])
