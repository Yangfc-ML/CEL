...
class CLASS_WISE_ENCODER(nn.Module):
    def __init__(self, num_classes, num_of_groups=-1, decoder_embedding=512, initial_num_features=2048):
        super(CLASS_WISE_ENCODER, self).__init__()
        embed_len_decoder = num_of_groups
        if embed_len_decoder > num_classes:
            embed_len_decoder = num_classes

        # switching to 512 initial embeddings
    ...
    def forward(self, x):  # class-wise embedding
        if len(x.shape) == 4:
            embedding_spatial = x.flatten(2).transpose(1, 2)
        else:
            embedding_spatial = x.transpose(1, 2)
        embedding_spatial_512 = self.decoder.embed_standart(embedding_spatial)
        embedding_spatial_512 = torch.nn.functional.relu(embedding_spatial_512, inplace=True)

        bs = embedding_spatial_512.shape[0]
        query_embed = self.decoder.query_embed.weight

        tgt = query_embed.unsqueeze(1).expand(-1, bs, -1)
        h = self.decoder(tgt, embedding_spatial_512.transpose(0, 1))
        h = h.transpose(0, 1)
        ...
        return F.normalize(h, p=2, dim=2), logits
